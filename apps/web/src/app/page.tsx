...
import { 
  Send, MessageSquare, Activity, Settings, FileCode, Copy, 
  Palette, Command, Zap, Plus, MoreHorizontal
} from "lucide-react";
...
export default function Dashboard() {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => prev.map(m => m.id === assistantId ? { ...m, role: "system", content: "Gate Offline" } : m));
    } finally {
      setIsThinking(false);
    }
  };
  ...
  return (
    ...
    >
      {/* Top Navigation Bar */}
      <header className="absolute top-0 left-0 right-0 h-16 border-b z-50 flex items-center justify-between px-8" style={{ borderColor: theme.border, backgroundColor: theme.bg }}>
        <div className="flex items-center gap-4">
           <Zap className="w-5 h-5" style={{ color: theme.accent }} />
           <span className="text-xs font-bold tracking-widest uppercase" style={{ color: theme.text }}>Rimuru Sovereign</span>
        </div>
        ...
        <div className="flex items-center gap-6">
           ...
           <div className="relative">
             ...
             <AnimatePresence>
               {showThemeMenu && (
                 <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-4 w-48 border rounded-lg shadow-xl overflow-hidden z-[100]" style={{ backgroundColor: theme.surface, borderColor: theme.border }}>
                   {ALL_THEMES.map(t => (
                     <button key={t.id} onClick={() => { setTheme(t); setShowThemeMenu(false); }} className="w-full flex items-center justify-between p-4 hover:bg-white/5 transition-colors text-xs border-b last:border-0" style={{ borderColor: theme.border }}>
                        <span>{t.name}</span>
  ...
  return (
    <ReactMarkdown
      ...
      components={{
        ...
        thead: ({ children }) => <thead className="bg-white/5 border-b" style={{ borderColor: theme.border }}>{children}</thead>,
        th: ({ children }) => <th className="px-6 py-3 text-left font-bold border-r last:border-r-0" style={{ borderColor: theme.border }}>{children}</th>,
        td: ({ children }) => <td className="px-6 py-3 border-r last:border-r-0 border-b last:border-b-0" style={{ borderColor: theme.border }}>{children}</td>,
        code({ node, inline, className, children, ...props }: any) {
          const match = /language-(\w+)/.exec(className || "");
          return !inline && match ? (
            <div className="my-8 border rounded overflow-hidden" style={{ borderColor: theme.border, backgroundColor: theme.surface }}>
              <div className="px-6 py-3 text-[10px] font-bold opacity-30 flex justify-between items-center border-b" style={{ borderColor: theme.border }}>
            ...
            <code className="px-1.5 py-0.5 rounded-sm bg-black/5 dark:bg-white/5 font-semibold" style={{ color: theme.accent }} {...props}>
              {children}
            </code>
          );
        },
