...
import { createRitual } from "../rituals/rituals.js";
const execFileAsync = promisify(execFile);
export const readFileRune = {
...
export const searchRune = {
    ...
    async invoke(input, context) {
        ...
        if (input.include)
            args.splice(3, 0, "--glob", input.include);
        try {
...
export const editFileRune = {
    ...
    async invoke(input, context) {
        ...
        if (!before.includes(input.find))
            return { path, changed: false, preview: "No match found" };
        const after = before.replace(input.find, input.replace);
        let rollbackPath;
        if (!input.dryRun) {
            rollbackPath = safeWorkspacePath(context, join(".rimuru/rollbacks", `${Date.now()}-${safeName(input.path)}.json`), true);
...
async function buildFileTreeString(dir, currentDepth, maxDepth) {
...
export const compileRune = {
    name: "workspace.compileRune",
    description: "Transpiles TypeScript to a sandboxed Rune stored in the workspace .rimuru/runes/ directory. The rune is executed in a QuickJS WebAssembly VM. Write standard JS/TS functions that receive normal parameters and return normal JS values.",
    risk: "write",
    inputSchema: {
        type: "object",
    ...
    async invoke(input, context) {
        ...
        const allowedRisks = ["read", "write", "execute", "network"];
