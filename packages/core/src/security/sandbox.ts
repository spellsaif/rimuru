...
export async function runSandboxedCommand(
  ...
  mode: SandboxMode = sandboxModeFromEnv(),
  ...
  if (mode === "wasi") {
    ...
    const stdoutPath = join(
      input.workspace,
      `.rimuru-wasi-stdout-${Date.now()}-${Math.random().toString(36).slice(2)}.log`,
    );
    const stderrPath = join(
      input.workspace,
      `.rimuru-wasi-stderr-${Date.now()}-${Math.random().toString(36).slice(2)}.log`,
    );
    const stdinPath = join(
      input.workspace,
      `.rimuru-wasi-stdin-${Date.now()}-${Math.random().toString(36).slice(2)}.log`,
    );
