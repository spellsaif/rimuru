 ...
 */
export async function createWorkspaceBranch(workspace, branchId, options = defaultOptions) {
 ...
 */
export async function deleteWorkspaceBranch(workspace, branchId) {
 ...
 */
export async function mergeWorkspaceBranch(workspace, branchId, options = defaultOptions) {
    const root = resolve(workspace);
    const branchDir = resolve(root, ".rimuru", "branches", branchId);
    if (!existsSync(branchDir))
        throw new Error(`Branch directory does not exist: ${branchDir}`);
    const mergedFiles = [];
    await mergeDirRecursive(branchDir, root, branchDir, root, options, mergedFiles);
    return mergedFiles;
}
async function copyDirRecursive(src, dest, srcRoot, destRoot, options) {
...
async function mergeDirRecursive(src, dest, srcRoot, destRoot, options, mergedFiles) {
    const entries = await readdir(src, { withFileTypes: true });
    const tasks = entries.map(async (entry) => {
        const srcPath = join(src, entry.name);
        const destPath = join(dest, entry.name);
