...
const adapterRegistry = new Map<string, ShardAdapter>();

export function registerShardAdapter(adapter: ShardAdapter): void {
...
export function createShard(config: RuntimeConfig): Shard {
  const adapter = adapterRegistry.get(config.provider as string);
  if (!adapter) {
    if ((config.provider as string) === "auto" && config.baseUrl) {
      for (const [kind, ad] of adapterRegistry) {
        if (kind === "mock") continue;
        if (ad.matches(config.baseUrl)) return ad.create(config);
      }
    }
    throw new Error(
      `Unknown or unsupported provider: ${config.provider}. Please configure a valid Shard (e.g., openai-compatible, openrouter, anthropic, gemini, ollama).`,
    );
  }
  return adapter.create(config);
}

export function listShardKinds(): readonly ProviderKind[] {
  return [...adapterRegistry.keys()] as ProviderKind[];
}
