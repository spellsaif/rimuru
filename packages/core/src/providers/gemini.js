export class GeminiShard {
    ...
    async complete(messages, options) {
        const payloadTools = options?.tools && options.tools.length > 0
            ...
            : undefined;
        const response = await this.#fetchWithRetry(`${this.#baseUrl}/models/${encodeURIComponent(this.#model)}:generateContent?key=${encodeURIComponent(this.#apiKey)}`, {
            method: "POST",
    ...
    async *stream(messages, options) {
        const payloadTools = options?.tools && options.tools.length > 0
            ...
            : undefined;
        const response = await this.#fetchWithRetry(`${this.#baseUrl}/models/${encodeURIComponent(this.#model)}:streamGenerateContent?alt=sse&key=${encodeURIComponent(this.#apiKey)}`, {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({
                contents: mapMessagesToGemini(messages),
        ...
        if (!response.body)
            throw new Error("Gemini stream failed: missing response body");
...
function mapSchemaToGemini(schema) {
    if (!schema)
        return undefined;
    const result = { type: String(schema.type).toUpperCase() };
    if (schema.required)
