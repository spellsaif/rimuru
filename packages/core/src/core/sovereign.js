...
export class Sovereign {
    ...
    async run(request) {
        ...
        const userMessage = request.promptMessage ?? {
            role: "user",
    ...
    async complete(messages, onText, tools) {
        if (!this.#shard.stream)
            return this.#shard.complete(messages, { tools });
        let content = "";
        let toolCalls;
        let usage;
