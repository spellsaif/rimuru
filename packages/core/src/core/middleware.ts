...
import type { Flow, PermissionPolicy, RuneInvocation, RuneMiddleware } from "./types.js";
...
export function auditMiddleware(options: AuditMiddlewareOptions = {}): RuneMiddleware {
...
export function permissionMiddleware(options: PermissionMiddlewareOptions): RuneMiddleware {
...
export function isolationMiddleware(): RuneMiddleware {
  return async (invocation, next) => {
...
export function skillMiddleware(skills: SkillRegistry): RuneMiddleware {
  let injected = false;
  return async (invocation, next) => {
    if (injected) return next();
    injected = true;
    const relevant = await skills.retrieveRelevant(
      invocation.context.state?.objective as string ?? invocation.name,
    );
    if (relevant.length > 0) {
      const skillText = skills.formatForPrompt(relevant);
      invocation.context = { ...invocation.context };
      (invocation.context as any).state = { ...invocation.context.state, skillContext: skillText };
    }
    return next();
  };
}
...
export function stagingMiddleware(store: StagingStore): RuneMiddleware {
  return async (invocation, next) => {
    const { risk } = invocation;
    if (risk !== "write" && risk !== "execute") return next();
    const output = await next();
...
export function learningMiddleware(workspace: string, skills: SkillRegistry): RuneMiddleware {
  let callCount = 0;
