...
export function parseUnifiedPatch(patch) {
    ...
    while (index < lines.length) {
        ...
        index += 1;
        if (!lines[index]?.startsWith("+++ "))
            throw new Error("Invalid unified patch: missing +++ file header");
        ...
        while (index < lines.length && !lines[index]?.startsWith("--- ")) {
            ...
            while (index < lines.length && !lines[index]?.startsWith("@@") && !lines[index]?.startsWith("--- ")) {
                ...
                if (line.startsWith("+"))
                    ...
                    throw new Error(`Invalid unified patch line: ${line}`);
            }
            hunks.push({ header, lines: hunkLines });
        }
        if (hunks.length === 0)
            throw new Error(`Invalid unified patch: no hunks for ${newPath}`);
        files.push({ oldPath, newPath, hunks });
    }
    if (files.length === 0)
        throw new Error("Invalid unified patch: no file changes found");
    return files;
}
export async function applyUnifiedPatch(options) {
...
export function applyPatchToText(before, file) {
    ...
    for (const hunk of file.hunks) {
        ...
        if (start === -1)
            throw new Error(`Patch conflict in ${file.newPath}: hunk does not match (${hunk.header})`);
...
function findSequence(source, expected, from) {
    if (expected.length === 0)
        return from;
    for (let index = from; index <= source.length - expected.length; index += 1) {
        if (expected.every((line, offset) => source[index + offset] === line))
            return index;
    }
    return -1;
}
function createUnifiedPreview(path, before, after) {
    if (before === after)
        return `--- ${path}\n+++ ${path}\nNo changes`;
