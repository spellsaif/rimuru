...
export function validateRuntimeConfig(
  ...
  if (
    (config.allowedRisks.includes("write") || config.allowedRisks.includes("execute")) &&
    ...
    diagnostics.push({
      ...
      message:
        "Write/execute vows are enabled without a Barrier. Consider barrier=readonly or docker for untrusted use.",
    });
  }
  ...
  for (const circle of config.circles) {
    ...
    if (
      (circle.kind === "telegram" || circle.kind === "slack" || circle.kind === "discord") &&
      circle.tokenEnv &&
      !env[circle.tokenEnv]
    ) {
      diagnostics.push({
  ...
  if (
    config.fallbackShards.some(
      (fallback) =>
        fallback.provider !== "mock" && fallback.provider !== "ollama" && !fallback.apiKey && !env.RIMURU_API_KEY,
    )
  ) {
    diagnostics.push({
      level: "info",
