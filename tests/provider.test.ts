...
describe("providers", () => {
  it("creates a mock shard from config", () => {
    const shard = createShard({ provider: "mock", model: "mock", sessionId: "s", memoryDir: "/tmp", allowedRisks: ["read"] });
  ...
  it("calls OpenAI-compatible chat completions", async () => {
    ...
    const fetchImpl = async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
      ...
      return Response.json({ choices: [{ message: { content: "hello from provider" } }], usage: { prompt_tokens: 3, completion_tokens: 4 } });
    };
    const shard = new OpenAICompatibleShard({ baseUrl: "https://example.test/v1/", apiKey: "secret", model: "m", fetchImpl });

    const response = await shard.complete([{ role: "user", content: "hi", createdAt: new Date("2026-01-01T00:00:00.000Z") }]);
  ...
  it("calls Anthropic messages API", async () => {
    const shard = new AnthropicShard({
      apiKey: "k",
      model: "m",
      fetchImpl: async () => Response.json({ content: [{ text: "claude" }], usage: { input_tokens: 1, output_tokens: 2 } })
    });
    await expect(shard.complete([{ role: "user", content: "hi", createdAt: new Date() }])).resolves.toEqual({ content: "claude", usage: { input: 1, output: 2 } });
  });
  ...
  it("calls Gemini generateContent API", async () => {
    ...
    await expect(shard.complete([{ role: "user", content: "hi", createdAt: new Date() }])).resolves.toEqual({ content: "gemini" });
  });
});
